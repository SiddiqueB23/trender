...
static void build_chunk_vertex_arrays(mesh_t* mesh, int start_index, int end_index, float* raw_vertices) {
	for (int i = start_index; i < end_index; i++) {
		int local_tri = i - start_index;
		for (int v = 0; v < 3; v++) {
			int vid = mesh->attrib.faces[i * 3 + v].v_idx;
			set_raw_vertex(raw_vertices, local_tri * 3 + v,
...
static inline clip_triangle_t process_triangle(float* clip_vertices, int local_tri_idx) {
	int base = local_tri_idx * 3;
	clip_triangle_t triangle;
	triangle.v0 = get_clip_vertex(clip_vertices, base + 0);
	triangle.v1 = get_clip_vertex(clip_vertices, base + 1);
	triangle.v2 = get_clip_vertex(clip_vertices, base + 2);
	return triangle;
}
...
typedef struct {
	...
	float*                raw_vertices;       /* SoA xyz, triangle-sequential: slot = local_tri*3+v */
	float*                clip_vertices;      /* SoA xyzw, groups of 8: x8 y8 z8 w8 */
	int                   num_vertices;       /* always num_triangles * 3 */
...
static inline primitive_pass_ctx_t create_primitive_pass_ctx(mesh_t* mesh, int start_index, int end_index, int num_chunks_y, int num_chunks_x) {
	...
	int num_groups    = (num_vertices + 7) / 8;
	ctx.triangles     = (clip_triangle_t*)malloc((size_t)num_triangles * sizeof(clip_triangle_t));
	ctx.raw_vertices  = (float*)malloc((size_t)num_groups * 24 * sizeof(float));
	ctx.clip_vertices = (float*)malloc((size_t)num_groups * 32 * sizeof(float));
	...
	build_chunk_vertex_arrays(mesh, start_index, end_index, ctx.raw_vertices);
...
void primitive_pass(mesh_t* mesh, primitive_pass_ctx_t* ctx, int start_index, int end_index) {
	...
	ctx->start_index = start_index;
	process_vertex_positions(ctx->raw_vertices, ctx->num_vertices, ctx->params.model_view_projection, ctx->clip_vertices);
	int num_cells = ctx->num_chunks_y * ctx->num_chunks_x;
	...
	for (int j = start_index; j < end_index; j+=64) {
		...
		for(int i = j; i < j + 64 && i < end_index ;i++) {
			clip_triangle_t tri = process_triangle(ctx->clip_vertices, i - start_index);
