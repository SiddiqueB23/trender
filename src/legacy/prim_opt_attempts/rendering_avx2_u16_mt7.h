	...
	for (int i = start_index; i < end_index; i++) {
		...
		for (int v = 0; v < 3; v++) {
			...
			for (int k = 0; k < win_count; k++) {
				if (win_vids[k] == vid) { local = win_locals[k]; break; }
			}
			if (local >= 0) {
				indices[local_tri * 3 + v] = local;
			} else {
				...
				if (win_count < VERTEX_WINDOW_SIZE) {
					win_vids[win_count]   = vid;
					win_locals[win_count] = num_unique;
					win_count++;
				} else {
					win_vids[win_tail]   = vid;
					win_locals[win_tail] = num_unique;
					win_tail = (win_tail + 1) % VERTEX_WINDOW_SIZE;
				}
				num_unique++;
			}
		}
