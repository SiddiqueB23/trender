...
typedef struct triangle_2d_vertex_t {
...
typedef struct triangle_2d_t {
    struct triangle_2d_vertex_t v0, v1, v2;
} triangle_2d_t;

float near_plane = 0.5f;
float far_plane = 10.0f;

// char debug_msgs[2048];
// int debug_msg_len = 0;

bool clip_triangles = true;

void get_barycentric_coordinates(struct triangle_2d_t triangle, float px, float py, float *alpha, float *beta, float *gamma) {
...
void get_barycentric_coordinates_2(struct triangle_2d_t triangle, float px, float py, float *u, float *v, float *w) {
...
void sample_texture_nearest(unsigned char *texture, int tex_width, int tex_height, float u, float v, float *r, float *g, float *b) {
    ...
    x = MIN(MAX(x, 0), tex_width - 1);
    y = MIN(MAX(y, 0), tex_height - 1);
...
void lerp_projected(struct triangle_2d_t triangle, float alpha, float beta, float gamma, triangle_2d_vertex_t *out) {
...
void lerp_vertex_pair(struct triangle_2d_vertex_t v0, struct triangle_2d_vertex_t v1, struct triangle_2d_vertex_t *out, float t) {
...
void draw_triangle(framebuffer_4i8 *fb, framebuffer_f *depth_buffer, struct triangle_2d_t triangle, unsigned char *texture, int tex_width, int tex_height) {
    ...
    int min_x = (int)(fmin(fmin(triangle.v0.mvpx, triangle.v1.mvpx), triangle.v2.mvpx) - 1.0f);
    int max_x = (int)(fmax(fmax(triangle.v0.mvpx, triangle.v1.mvpx), triangle.v2.mvpx) + 1.0f);
    int min_y = (int)(fmin(fmin(triangle.v0.mvpy, triangle.v1.mvpy), triangle.v2.mvpy) - 1.0f);
    int max_y = (int)(fmax(fmax(triangle.v0.mvpy, triangle.v1.mvpy), triangle.v2.mvpy) + 1.0f);
    min_x = MIN(MAX(min_x, 0), fb->width - 1);
    max_x = MIN(MAX(max_x, 0), fb->width - 1);
    min_y = MIN(MAX(min_y, 0), fb->height - 1);
    max_y = MIN(MAX(max_y, 0), fb->height - 1);
    ...
    for (int y = min_y; y <= max_y; y++) {
        for (int x = min_x; x <= max_x; x++) {
            ...
            if (alpha >= 0 && beta >= 0 && gamma >= 0) {
                ...
                // r = (unsigned char)MIN(MAX(lroundf(interp.mvx * 255.0f), 0), 255);
                // g = (unsigned char)MIN(MAX(lroundf(interp.mvy * 255.0f), 0), 255);
                // b = (unsigned char)MIN(MAX(lroundf(interp.mvz * 255.0f), 0), 255);
                ...
                r = (unsigned char)MIN(MAX(lroundf(frag_color[0] * 255.0f), 0), 255);
                g = (unsigned char)MIN(MAX(lroundf(frag_color[1] * 255.0f), 0), 255);
                b = (unsigned char)MIN(MAX(lroundf(frag_color[2] * 255.0f), 0), 255);
                float current_depth;
...
void rasterize_mesh(tinyobj_attrib_t attrib, mat4 model_view_projection, mat3 normal_transfrom, mat4 model_view,
    ...
    for (int i = 0; i < attrib.num_face_num_verts; i++) {
        ...
        if (clip_triangles) {
            ...
            // debug_msg_len += sprintf(debug_msgs + debug_msg_len, "Clipped into %d triangle(s)\r\n", num_clipped_triangles);
            ...
            // debug_msg_len += sprintf(debug_msgs + debug_msg_len, "Clipping Disabled\r\n");
            draw_triangle(fb, depth_buffer, triangle, texture, tex_width, tex_height);
        }
    }
}
...
int main() {
    ...
    while (1) {
        ...
        display_framebuffer_4i8_sixel_ordered_dithering_optimized(fb, 5);
