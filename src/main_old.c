...
int main() {
	...
	framebuffer_u16 fb2 = create_framebuffer_u16(cols, rows);
	...
	while (num_frame_counter--) {
		...
		//clear_framebuffer_u16(&fb2, convert_8r8g8b8a_to_5r6g5b(255, 0, 255));
		clear_framebuffer_i32(&ib, -1);
		//render_mesh(&mesh, model_view_projection, normal_matrix, model_view, &fb, &depth_buffer, &ib);
		render_mesh(&mesh, model_view_projection, normal_matrix, model_view, &fb2, &depth_buffer, &ib);
		...
		convert_5r6g5b_to_sixel_indexed_bitmap_rgbuniform_ordered_dithering_216colors(&sixel_ctx.bitmap, fb2);
	...
	printf("Display:       %0.2f\r\n", total_display_time);
	printf("Tex Sampling:  %0.2f\r\n", total_texture_sample_time);
	printf("Frame time:    %0.2f\r\n", total_frame_time);
	printf("Average times:\r\n");
	printf("Rasterization: %0.2f\r\n", total_rasterization_time / (float)num_frames);
	printf("Conversion:    %0.2f\r\n", total_conversion_time / (float)num_frames);
	printf("Generation:    %0.2f\r\n", total_generation_time / (float)num_frames);
	printf("Display:       %0.2f\r\n", total_display_time / (float)num_frames);
	printf("Tex Sampling:  %0.2f\r\n", total_texture_sample_time / (float)num_frames);
