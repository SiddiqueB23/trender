...
void sample_texture_nearest(unsigned char* texture, int tex_width, int tex_height, float u, float v, float* r, float* g, float* b) {
	...
	*r = gamma_lut[texture[index + 0]];
	*g = gamma_lut[texture[index + 1]];
	*b = gamma_lut[texture[index + 2]];
}

void lerp_projected_depth(triangle_2d_t triangle, float alpha, float beta, float gamma, triangle_2d_vertex_t* out) {
...
void lerp_projected_attrs(triangle_2d_t triangle, float alpha, float beta, float gamma, triangle_2d_vertex_t* out) {
...
void lerp_projected_attrs2(triangle_2d_t triangle, float alpha, float beta, float gamma, triangle_2d_vertex_t* out) {
	float* out_buf = (float*)out;
	float* v0_buf = (float*)(&(triangle.v0));
	float* v1_buf = (float*)(&(triangle.v1));
	float* v2_buf = (float*)(&(triangle.v2));
	float coeff0 = alpha / triangle.v0.mvpw;
	float coeff1 = beta / triangle.v1.mvpw;
	float coeff2 = gamma / triangle.v2.mvpw;
	float coeff3 = out->mvpw;
	for (int i = 4;i < 16;i++) {
		out_buf[i] = coeff3 * (coeff0 * v0_buf[i] + coeff1 * v1_buf[i] + coeff2 * v2_buf[i]);
	}
	vec3 normal = { out->nx, out->ny, out->nz };
	...
	for (int y = min_y; y <= max_y; y++) {
		for (int x = min_x; x <= max_x; x++) {
			...
			if (alpha >= 0 && beta >= 0 && gamma >= 0) {
				...
				float current_depth = interp.mvpw;
				get_pixel_f(depth_buffer, x, y, &current_depth);
				//if (interp.mvpw > current_depth) continue;
				if (interp.mvpw > current_depth) continue;
				//if (interp.mvpz > 1.0f) continue;
				//if (interp.mvpz < -1.0f) continue;
				lerp_projected_attrs2(triangle, alpha, beta, gamma, &interp);
				...
				//float depth = 1.0/current_mvpw;
				//if (depth > 1.0)depth = 0.0;
				//float depth = interp.mvpwqqqqqqqqqqqqqqqqpth, depth, depth }, frag_color);
				//depth = depth * 0.5f + 0.5f;
				//glm_vec3_copy(diffuse_color, frag_color);
				//glm_vec3_copy((vec3) { 0.0, 1.0, 0.0 }, frag_color);
				...
				set_pixel_f(depth_buffer, x, y, interp.mvpw);
